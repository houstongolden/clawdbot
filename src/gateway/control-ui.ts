...
function injectControlUiConfig(html: string, opts: ControlUiInjectionOpts): string {
  ...
  const name = assistantName ?? DEFAULT_ASSISTANT_IDENTITY.name;
  const avatar = assistantAvatar ?? DEFAULT_ASSISTANT_IDENTITY.avatar;
  const script =
    `<script>` +
    `window.__OPENCLAW_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePath)};` +
    // Inject both Myo and OpenClaw naming for compatibility
    `window.__MYO_ASSISTANT_NAME__=${JSON.stringify(name)};` +
    `window.__MYO_ASSISTANT_AVATAR__=${JSON.stringify(avatar)};` +
    `window.__OPENCLAW_ASSISTANT_NAME__=${JSON.stringify(name)};` +
    `window.__OPENCLAW_ASSISTANT_AVATAR__=${JSON.stringify(avatar)};` +
    `</script>`;
  // Check if already injected
  if (html.includes("__MYO_ASSISTANT_NAME__") || html.includes("__OPENCLAW_ASSISTANT_NAME__"))
    return html;
